...
namespace Pacman
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (isPlaying)
            {
                ...
                if (Console.KeyAvailable) //Get the input when the key is pressed
                {
                    ...
                    ChangeDirection(ref pacmanDirX, ref pacmanDirY, key);
                }
                if (map[pacmanX + pacmanDirX, pacmanY + pacmanDirY] != '#')
                {
                    collectedDots = CollectDots(pacmanX, pacmanY, collectedDots, map);
                    Movement(map, '@', ref pacmanX, ref pacmanY, pacmanDirX, pacmanDirY);
                }
                if (map[ghostX + ghostDirX, ghostY + ghostDirY] != '#')
                {
                    Movement(map, '$', ref ghostX, ref ghostY, ghostDirX, ghostDirY);
                }
                else
                {
                    ChangeDrection(random, ref ghostDirX, ref ghostDirY);
                }
        ...
        private static int CollectDots(int pacmanX, int pacmanY, int collectedDots, char[,] map)
        {
        ...
        static void ChangeDrection(Random random, ref int DX, ref int DY)
        {
        ...
        private static void Movement(char[,] map, char symbol, ref int X, ref int Y, int DX, int DY)
        {
        ...
        static char[,] ReadMap(string mapName, out int pacmanX, out int pacmanY, ref int allDots, out int ghostX, out int ghostY)
        {
            pacmanX = 0;
            pacmanY = 0;
            ghostX = 0;
